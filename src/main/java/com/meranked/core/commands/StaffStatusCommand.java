...
public final class StaffStatusCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        sender.sendMessage("§6§lStaff Status");
        sender.sendMessage("§7Bridge: " + (bridge.isConnected() ? "§aConnected" : "§cDisconnected"));
        sender.sendMessage("§7Queued events: §f" + bridge.queueSize());
        sender.sendMessage("§7Server ID: §f" + bridge.config().serverId());
        if (sender instanceof Player player) {
            sender.sendMessage("§7Your duty: " + (services.staffDuty().isOnDuty(player.getUniqueId())
                    ? "§aOn" : "§7Off"));
        }
        sender.sendMessage("§7Staff on duty (§f" + services.staffDuty().onDutyCount() + "§7):");
